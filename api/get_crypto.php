...
function getCryptoData(&$dataSource = null, &$apiStatus = null, &$errorMessage = null)
{
    ...
    if (file_exists($cacheFile) && (time() - filemtime($cacheFile)) < $cacheTime) {
        ...
        if (is_array($cachedData) && !empty($cachedData)) {
            ...
            if ($apiStatus !== null) $apiStatus = "cached";
            return $cachedData;
        }
    }
    ...
    // 3. Penanganan Error & Status Koneksi API
    if ($err || $httpCode != 200) {
        if ($err) {
            if ($apiStatus !== null) $apiStatus = "error_connection";
            if ($errorMessage !== null) $errorMessage = "Koneksi ke API terputus atau gagal ($err).";
        } elseif ($httpCode == 429) {
            if ($apiStatus !== null) $apiStatus = "error_rate_limit";
            if ($errorMessage !== null) $errorMessage = "Terlalu banyak permintaan ke API (Error HTTP 429 Rate Limit).";
        } else {
            if ($apiStatus !== null) $apiStatus = "error_http";
            if ($errorMessage !== null) $errorMessage = "Server API mengembalikan kode error HTTP $httpCode.";
        }
        ...
        if (file_exists($cacheFile)) {
            $cachedData = json_decode(file_get_contents($cacheFile), true);
            if (is_array($cachedData) && !empty($cachedData)) {
                if ($dataSource !== null) $dataSource = "Cache Lokal (Mode Fallback)";
                return $cachedData;
            }
        }
        
        // Jika cache belum ada, gunakan Fallback Snapshot Data
        if ($dataSource !== null) $dataSource = "Snapshot Cadangan (Mode Offline)";
    ...
    if (!$data || !is_array($data)) {
        if ($apiStatus !== null) $apiStatus = "error_json";
        if ($errorMessage !== null) $errorMessage = "Respons API tidak valid atau format data rusak.";
    ...
    if ($apiStatus !== null) $apiStatus = "online";
